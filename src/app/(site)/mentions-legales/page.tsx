 ...
 *
 * Les mentions marquées « [À COMPLÉTER] » proviennent des réglages du
 * dashboard (groupes `contact` et `legal`, écrans `/dashboard/reglages/
 * contact` et `/dashboard/reglages/legal` depuis le Lot 10) : il suffit de
 * les y renseigner une seule fois pour qu'elles apparaissent ici et partout
 * ailleurs sur le site.
 *
 * ⚠️  `force-dynamic` — TRANSITOIRE, À RETIRER AU LOT 15. Sans elle, la page
 * serait figée au build (mesuré : `○ /mentions-legales`) et un numéro
 * d'enregistrement renseigné depuis le dashboard n'apparaîtrait qu'au
 * prochain déploiement — voir `(site)/page.tsx`.
 */
export const dynamic = "force-dynamic";

export default async function MentionsLegalesPage() {
  const [identite, contact, legal] = await Promise.all([
    getIdentitySettings(),
    getContactSettings(),
    getLegalSettings(),
  ]);
  ...
  return (
    <LegalPage title="Mentions légales" updatedAt="2026-08-27">
      <LegalSection title="Éditeur du site">
        <p>
          Le présent site est édité par <strong>{identite.name}</strong> —{" "}
          {identite.legalName}, association camerounaise à but non lucratif.
        </p>
        <ul>
      ...
      <LegalSection title="Propriété intellectuelle">
        <p>
          ...
          {identite.name} ou de ses partenaires, sauf mention contraire.
        </p>
        <p>
      ...
      <LegalSection title="Responsabilité">
        <p>
          {identite.name} s&apos;efforce de maintenir les informations
        ...
        <p>
          ...
          plateformes vidéo). {identite.name} n&apos;exerce aucun contrôle sur
